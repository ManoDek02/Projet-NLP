...
from src.models.schemas import ChatRequest, SearchResult, Conversation
...
class TestChatbotService:
    ...
    def test_chat_with_llm(self, chatbot_service, mock_services):
        ...
        vector_store.search.return_value = [
            SearchResult(conversation=conv, score=0.95, rank=1)
        ]
    ...
    def test_chat_no_results(self, chatbot_service, mock_services):
        ...
        # Assert
        assert response is not None
        assert "couldn't find" in response.message.lower() or "no relevant" in response.message.lower()
